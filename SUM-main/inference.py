...
def setup_model(device):
    ...
    if config.network == 'sum':
        ...
        model.load_state_dict(torch.load('./net/pre_trained_weights/SUM_newK5.pth', map_location=device))
        model.to(device)
        return model
    else:
...
def main():
    ...
    hot_output_filename = os.path.join(args.output_path, f'{filename}.png')
