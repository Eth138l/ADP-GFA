...
def gaussian_filter_1d(tensor, dim, sigma, truncate=4, kernel_size=None, padding_mode='replicate', padding_value=0.0):
    ...
    # print(f"Moving dimension: {dim}")

    # Check if dim is within range
    if dim < 0 or dim >= len(source_shape):
        raise ValueError(f"Dimension {dim} is out of range for tensor with {len(source_shape)} dimensions.")

    tensor = torch.movedim(tensor, dim, len(source_shape)-1)
    # print(f"Tensor shape after movedim: {tensor.shape}")
    ...
    def forward(self, tensor):
        ...
        for dim in self.dims:
            if dim < 0 or dim >= len(tensor.shape):
                raise ValueError(f"Dimension {dim} is out of range for tensor with {len(tensor.shape)} dimensions.")
            ...
            # print(f"Tensor shape after gaussian_filter_1d: {tensor.shape}")
